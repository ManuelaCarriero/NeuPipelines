...
%% Estimate regional medians (I method: remove voxels with zero values of CBF and CMRO2 in all maps, both energetic and microstructural)
...
%% Method II (remove zeros of binary maps)
V_GM = V_GM_tot;
V_GM(V_GM>0.5)=1;
V_GM(V_GM<1)=0;
GM_zeros=find(V_GM==0);
...
for i = 1:1:n_subjs %1:1:length(lst)
    ...
    for k = 2:numel(regions)
        ...
        v_fc_masked(tot_zeros_unique)=[];
        v_fsoma_masked(tot_zeros_unique)=[];
        v_rsoma_masked(tot_zeros_unique)=[];
        v_fsup_masked(tot_zeros_unique)=[];
        v_fneurite_masked(tot_zeros_unique)=[];
        v_fextra_masked(tot_zeros_unique)=[];
        v_Din_masked(tot_zeros_unique)=[];
        v_De_masked(tot_zeros_unique)=[];
...
%% Method III (remove zeros of binary maps in all maps and CBF<=0, CMRO2<=0)
V_GM = V_GM_tot;
V_GM(V_GM>0.5)=1;
V_GM(V_GM<1)=0;
GM_zeros=find(V_GM==0);
...
for i = 1:1:n_subjs %1:1:length(lst)
    ...
    for k = 2:numel(regions)
        ...
        v_CBF_masked(v_CBF_masked<=0)=[];
        v_CMRO2_masked(v_CMRO2_masked<=0)=[];
...
for i = 1:1:n_subjs %1:1:length(lst)
    ...
    for k = 2:numel(regions)
        ...
        V_CBF_withoutzerosatlas(atlas_zeros)=[];
        V_CMRO2_withoutzerosatlas(atlas_zeros)=[];
...
%% Estimate regional medians (V method: remove voxels with <=0 values of CBF and CMRO2 in all maps, both energetic and microstructural)
...
%% Method VI (remove zeros of binary maps in all maps and CBF=0, CMRO2=0 in all maps)
V_GM = V_GM_tot;
V_GM(V_GM>0.5)=1;
V_GM(V_GM<1)=0;
GM_zeros=find(V_GM==0);
...
%% Method VII (remove zeros of binary maps in all maps and CBF=0, CMRO2=0)
V_GM = V_GM_tot;
V_GM(V_GM>0.5)=1;
V_GM(V_GM<1)=0;
GM_zeros=find(V_GM==0);
